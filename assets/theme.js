                    ...
                    if ($('.header-mobile.header-mobile-style-3 .header-mobile-search').length > 0 ){
                        if (!$('.header-mobile-search .header__seach--details').length){
                            ...
                            if($('.header .header__search').hasClass('show-icon')) {
                                ...
                                $('.header .header__search').find('.header__seach--details.predictive-search-form').appendTo($('.header-mobile-search .header__search--mobile'));
                            }
                        }
                    } else {
                        if (!$('#search-form-mobile .halo-sidebar-wrapper').find('.header__seach--details').length){
                            ...
                            if($('.header .header__search').hasClass('show-icon')) {
                                ...
                                $('.header .header__search').find('.header__seach--details.predictive-search-form').appendTo($('#search-form-mobile .halo-sidebar-wrapper'));
                            }
                        }
                    }
                } else {
                                        ...
                                        success: (data) => {
                                            if (url != '') {
                                                if (layout == 'grid') {
                                                    ...
                                                    // $block.find('.products-scroll').html(data);
                                                }
                                            }
                                        },
                                        complete: () => {
        ...
        buildRecentlyViewedBlock: function(){
            ...
            if(productBlock.length > 0){
                ...
                if (list.length == 0){
                    ...
                    if (!productBlock.hasClass('ajax-loaded')) {
                        var layout = productBlock.data('layout'),
                            ...
                            sectionId = productBlock.attr('sectionId');
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                constraint: `template=section+limit=${limit}+layout=${layout}+sectionId=recently-viewed-section+imageRatio=${imageRatio}+action=${action}+portraitAspectRatio=${ratio}+list_handle=` + encodeURIComponent(list)
                            },
                            beforeSend: () => {
                            ...
                            success: (data) => {
                                if (layout == 'grid') {
                                    productBlock.find('.products-grid').html(data);
                                } else if (layout == 'slider'){
                                    productBlock.find('.products-carousel').html(data);
                                }
                            },
                            complete: () => {
                                if (layout == 'slider') {
        ...
        setProductForCompare: function() {
            $doc.on('click', '[data-compare-link]', (event) => {
                ...
                if (compareList.length <= 1) {
                    ...
                    halo.updateContentCompareProduct(compareList);
                }
            });
        ...
        setProductForWishlistPage2: function(list){
            ...
            $.ajax({
                ...
                data: {
                    ...
                    constraint: `limit=${list.length}+sectionId=list-wishlist+imageRatio=${imageRatio}+portraitAspectRatio=${ratio}+list_handle=` + encodeURIComponent(list)
                },
                success: (data) => {
